...
export const masterEditionFilter = ({ edition }: SafetyDepositDraft): boolean => !edition

export const limitedSupplyFilter = ({ masterEdition }: SafetyDepositDraft): boolean =>
  !!masterEdition?.info.maxSupply?.toNumber()

export const unlimitedSupplyFilter = ({ masterEdition }: SafetyDepositDraft): boolean =>
  masterEdition?.info.maxSupply === undefined

export const hasSupplyFilter = ({ masterEdition }: SafetyDepositDraft): boolean =>
  !!masterEdition?.info.supply?.toNumber()
...
export const vouchersFilter =
  (selectedItems: Record<string, SafetyDepositDraft>) => (item: SafetyDepositDraft) =>
...
export const mapSelectedItems = ({
  ...
  Object.keys(selectedItems).map((pubKey, index) => {
    ...
    const maxSupply =
      distributionType !== PackDistributionType.Unlimited && supplyByMetadataKey[pubKey]
        ? new BN(supplyByMetadataKey[pubKey])
        : new BN(0)
    const weight =
      distributionType !== PackDistributionType.MaxSupply && weightByMetadataKey[pubKey]
