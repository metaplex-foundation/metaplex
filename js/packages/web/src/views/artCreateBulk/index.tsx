...
export const ArtCreateBulkView = () => {
  ...
  const gotoStep = useCallback(
    (_step: number) => {
      history.push(`/art/create-bulk/${_step.toString()}`);
...
// const imageUrl = 'https://pbs.twimg.com/media/E9FTupLUUAMgW7O?format=jpg&name=large';
const imageUrl = '/generated-birdz/E87fDVEWQAImNEU.png';
...
const UploadStep = (props: {
  ...
  const createFile = async () => {
    ...
    if (file) {
      ...
      props.setFiles([file].filter(f => f) as File[]);
      props.confirm();
    } else {
  ...
  return (
    <>
      <Row className="call-to-action" style={{ marginBottom: 20 }}>
        <h2>Connect wallet to set royalties and creator splits</h2>
        <p>
