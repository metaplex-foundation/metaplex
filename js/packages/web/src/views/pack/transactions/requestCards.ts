...
import { RequestCardsParams, RequestCardsInstructionsParams, RequestCardParams } from './interface'
...
export const requestCards = async ({
  ...
  const requestCardsInstructionsSigners: Keypair[][] = requestCardsInstructionsChunks.map(() => [])
...
const getRequestCardsInstructions = async ({
  ...
  const addCardsToPack = Array.from({ length: cardsLeftToOpen }).map((_, index) =>
