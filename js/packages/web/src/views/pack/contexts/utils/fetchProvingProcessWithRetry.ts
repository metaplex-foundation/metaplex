import { getUnixTs, ParsedAccount, sleep, StringPublicKey } from '@oyster/common'
import {
...
export const fetchProvingProcessWithRetry = async ({
  provingProcessKey,
  connection,
}: FetchProvingProcessWithRetryParams): Promise<ParsedAccount<ProvingProcess>> => {
  let provingProcess
  const startTime = getUnixTs()

  while (!provingProcess && getUnixTs() - startTime < REQUEST_TIMEOUT) {
    try {
      provingProcess = await getProvingProcessByPubkey(connection, provingProcessKey)
    } catch {
      // skip
    }
