...
import { GetProvingProcessParams, RequestCardsUsingVoucherParams } from '../interface'
...
export const getProvingProcess = async ({
  ...
  const voucher = Object.values(vouchers).find(({ info }) => info.master === edition.parent)
  ...
  const alreadyRequestedCards = provingProcess?.info.cardsToRedeem
    ? Object.values(Object.fromEntries(provingProcess.info.cardsToRedeem)).reduce((a, b) => a + b)
    : 0
