import { findPackCardProgramAddress, StringPublicKey, toPublicKey } from '@oyster/common'
...
export const getMetadataUserToReceive = async ({
  ...
  for (const [index, numberToRedeem] of cardsToRedeem.entries()) {
    const packCard = await findPackCardProgramAddress(toPublicKey(packPubKey), index)
