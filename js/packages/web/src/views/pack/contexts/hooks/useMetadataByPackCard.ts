import { Metadata, PackDistributionType, ParsedAccount, useMeta } from '@oyster/common'
...
export const useMetadataByPackCard = (packId: string): PackMetadataByPackCard => {
  ...
  const isMaxSupplyDistribution = distributionType === PackDistributionType.MaxSupply
  const total = isMaxSupplyDistribution ? totalEditions : totalWeight
  const cards = packCardsByPackSet[packId]

  const metadata = cards?.reduce<PackMetadataByPackCard>((packMetadata, { info, pubkey }) => {
