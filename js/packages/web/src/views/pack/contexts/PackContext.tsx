...
export const PackProvider: React.FC = ({ children }) => {
  ...
  const [isPollingProvingProcess, setIsPollingProvingProcess] = useState<boolean>(false)

  const [provingProcess, setProvingProcess] = useState<ParsedAccount<ProvingProcess>>()
  const [redeemModalMetadata, setRedeemModalMetadata] = useState<string[]>([])

  const voucherMetadata = metadata.find(meta => meta?.info?.mint === voucherMint)
  const voucher = Object.values(vouchers).find(voucher => voucher?.info?.packSet === packKey)
