...
const ClaimingStep: React.FC<ClaimingStepProps> = ({ onClose }) => {
  ...
  const { pack, voucherMetadataKey, provingProcess, redeemModalMetadata } = usePack()
  ...
  const currentMetadataToShow =
    currentCardIndex >= 0 ? redeemModalMetadata[currentCardIndex] : voucherMetadataKey
      ...
      <div className='claiming-step__cards-container'>
        ...
        <div className='current-card-container'>
          <SwitchTransition>
            ...
            >
              <ArtContent key={currentCardIndex} pubkey={currentMetadataToShow} preview={false} />
            </CSSTransition>
          </SwitchTransition>
        </div>
      </div>
