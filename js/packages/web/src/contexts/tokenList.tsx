...
const TokenListContext = React.createContext<TokenListContextState | null>(null)

export function SPLTokenListProvider({ children = null }: { children: React.ReactNode }) {
  ...
  const subscribedTokens = tokenList
    ? tokenList.getList().filter(f => subscribedTokenMints.some(s => s == f.address))
    : []
