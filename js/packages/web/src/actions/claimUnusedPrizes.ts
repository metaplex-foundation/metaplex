...
> {
  ...
  for (let i = 0; i < bids.length; i++) {
    ...
    if (!bid.info.cancelled) {
      ...
      const bidRedemption =
        bidRedemptions[await getBidRedemption(auctionView.auction.pubkey, bid.pubkey)]
...
export async function claimUnusedPrizes(
  ...
  if (
    ...
    auctionView.participationItem.masterEdition?.info.key == MetadataKey.MasterEditionV2
  ) {
    ...
    if (balance.value.uiAmount || 0 > 0) {
      ...
      const unredeemedParticipations = await findEligibleParticipationBidsForRedemption(
        auctionView,
        bids,
        bidRedemptions
      )
