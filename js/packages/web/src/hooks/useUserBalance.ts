...
export function useUserBalance(mintAddress?: StringPublicKey, account?: StringPublicKey) {
  const mint = useMemo(
  ...
  const altSplPrice = useAllSplPrices().filter(a => a.tokenMint == mintAddress)[0]?.tokenPrice
  const tokenPrice = mintAddress == WRAPPED_SOL_MINT.toBase58() ? solPrice : altSplPrice

  const mintInfo = useMint(mint)
  const accounts = useMemo(() => {
    return userAccounts
      .filter(acc => mint === acc.info.mint.toBase58() && (!account || account === acc.pubkey))
