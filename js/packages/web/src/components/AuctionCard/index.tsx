...
async function calculateTotalCostOfRedeemingOtherPeoplesBids(
  ...
  const metadataRentExempt = await connection.getMinimumBalanceForRentExemption(MAX_METADATA_LEN);
  const editionRentExempt = await connection.getMinimumBalanceForRentExemption(MAX_EDITION_LEN);
  const prizeTrackingTicketExempt = await connection.getMinimumBalanceForRentExemption(
    MAX_PRIZE_TRACKING_TICKET_SIZE
  );

  const eligibleParticipations = await findEligibleParticipationBidsForRedemption(
    auctionView,
    bids,
    bidRedemptions
  );
...
function useAuctionExtended(
  ...
  const [auctionExtended, setAuctionExtended] = useState<ParsedAccount<AuctionDataExtended>>();
...
export const AuctionCard = ({
  ...
  const isAuctioneer =
    wallet?.publicKey && auctionView.auctionManager.authority === connectedWalletPublickKey;
  ...
  let minBid = fromLamports(participationOnly ? participationFixedPrice : priceFloor, mintInfo);
  ...
  const biddingPower =
    balance.balance +
    (auctionView.myBidderMetadata && !auctionView.myBidderMetadata.info.cancelled
  ...
  const instantSale = async () => {
    ...
    try {
      // Step 1: Setup
      const instantSalePrice = auctionView.auctionDataExtended?.info.instantSalePrice;
          ...
          const tryPatchMeta = async (txid: string) => {
            for (let i = 0; i < 5; ++i) {
...

              {
                ...
                auctionView.myBidderPot = patch.bidderPotsByAuctionAndBidder[auctionBidderKey];
                auctionView.myBidderMetadata =
  ...
  const isOpenEditionSale = auctionView.auction.info.bidState.type === BidStateType.OpenEdition;
  ...
  const shouldHide =
    ...
    (auctionView.vault.info.state === VaultState.Deactivated && isBidderPotEmpty);
  ...
  const loadingRedeemReclaimRefundBtn =
    loading || auctionView.items.find((i) => i.find((it) => !it.metadata)) || !myPayingAccount;
        ...
        onClick={async () => {
          ...
          if (eligibleForAnything) {
            try {
              ...
              notification.success({
                ...
                description: (
                  <Space direction="vertical">
                    <Text>
                      Congratulations, your bid was redeemed for the NFT! See it in your owned tab.
                    </Text>
                    <Button type="primary">View Owned</Button>
                  </Space>
                ),
            ...
            try {
              ...
              const message = cancelBidMessages[isAuctioneer ? 'reclaimed' : 'refunded'];
              ...
              notification.error({
                message: isAuctioneer ? 'Reclaim Items Error' : 'Bid Refund Error',
                description:
                  (isAuctioneer
  ...
  const showStartAuctionBtn = isAuctionNotStarted && !isAuctionManagerAuthorityNotWalletOwner;

  // If the user can start this auction, or a bid can be placed on it
  const showStartOrPlaceBidBtns =
    showDefaultNonEndedAction && (showStartAuctionBtn || !auctionView.isInstantSale);
  ...
  const StartAuctionBtn = (
    <Button
      ...
      onClick={async () => {
        setLoading(true);
        try {
          await startAuctionManually(connection, wallet, auctionView.auctionManager.instance);
        } catch (e) {
          console.error(e);
        }
        setLoading(false);
    ...
    <Space className="metaplex-fullwidth metaplex-space-align-stretch" direction="vertical">
      ...
      <Row gutter={8} align="middle">
        ...
        <Col flex="0 0 auto">
          ...
          <Button disabled={loading} type="text" onClick={() => setShowPlaceBidUI(false)}>
            Cancel
          </Button>
        </Col>
      </Row>
            ...
            <>
              {notEnoughFundsToBid && triedToBid && (
                <Text className="danger" type="danger">
                  You do not have enough funds to fulfill the bid. Your current bidding power is{' '}
                  {biddingPower} SOL.
                </Text>
              )}
              ...
              {gapBidInvalid && (
                <Text className="danger" type="danger">
                  Your bid needs to be at least {gapTick}% larger than an existing bid during gap
                  periods to be eligible.
                </Text>
              )}
            </>
          )}
  ...
  const auctionEnded = auctionView.isInstantSale ? undefined : isAuctionOver(auctionView);

  const someoneWon = auctionEnded && bids.length;

  const showHowAuctionsWorkBtn = !showPlaceBidUI && !auctionView.isInstantSale;

  const actuallyShowStartAuctionBtn = showDefaultNonEndedAction && showStartAuctionBtn;
  ...
  const showPlaceBidButton = !showPlaceBidUI && showStartOrPlaceBidBtns && !showStartAuctionBtn;
  ...
  return (
    <div>
      ...
      >
        ...
        {someoneWon && mint
          ? winners.map((bid) => (
              <WinnerProfile bidderPubkey={bid.info.bidderPubkey} key={bid.info.bidderPubkey} />
            ))
          : null}
      ...
      <MetaplexModal visible={showWarningModal} onCancel={() => setShowWarningModal(false)}>
        <h3>
          ...
          are agreeing to foot the cost of up to an estimated ◎
          <b>{(printingCost || 0) / LAMPORTS_PER_SOL}</b> plus transaction fees to redeem their bids
          for them right now.
        </h3>
      </MetaplexModal>
    </div>
...
const WinnerProfile = ({ bidderPubkey }: { handle?: string; bidderPubkey: string }) => {
  const connection = useConnection();
