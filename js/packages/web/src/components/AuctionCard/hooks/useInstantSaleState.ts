...
export const useInstantSaleState = (auctionView: AuctionView): ActionButtonContentProps => {
  ...
  const { isInstantSale, auctionManager, auction, myBidRedemption, myBidderPot, myBidderMetadata } =
    auctionView
  ...
  if (auctionView.auction.info.bidState.type == BidStateType.EnglishAuction) {
    for (const item of items) {
      for (const subItem of item) {
        const bidRedeemed = myBidRedemption?.info.getBidRedeemed(subItem.safetyDeposit.info.order)
