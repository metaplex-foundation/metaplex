...
export const ENDPOINTS: Array<Endpoint> = [
  {
    name: 'mainnet-beta',
    label: 'mainnet-beta',
    url: 'https://solana-api.projectserum.com', // clusterApiUrl('mainnet-beta'), //
  ...
  },
  {
    name: 'devnet',
    label: 'devnet',
    url: clusterApiUrl('devnet'), //'http://localhost:8899', //
...
export function ConnectionProvider({ children }: { children: any }) {
  ...
  const [networkStorage, setNetworkStorage] = useLocalStorageState<ENDPOINT_NAME>(
    'network',
  ...
  useEffect(() => {
    function fetchTokens() {
      return getTokenListContainerPromise().then(container => {
        const list = container.excludeByTag('nft').filterByChainId(endpoint.chainId).getList()
