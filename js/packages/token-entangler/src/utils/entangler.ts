...
export const searchEntanglements = async (
    ...
    console.log('Loading token entanglement program...');
    const anchorProgram = await loadTokenEntanglementProgram(anchorWallet, connection);
    console.log('Token entanglement program loaded.');

    const searchMint = new PublicKey(mint);

    const searchMintAAccounts = await anchorProgram.provider.connection.getProgramAccounts(TOKEN_ENTANGLEMENT_PROGRAM_ID, {
        filters: [
            // Filter for MintA
            {
                memcmp: {
    ...
    const searchMintBAccounts = await anchorProgram.provider.connection.getProgramAccounts(TOKEN_ENTANGLEMENT_PROGRAM_ID, {
        filters: [
            // Filter for MintB
            {
                memcmp: {
    ...
    const entanglementsAccounts = [...searchMintAAccounts, ...searchMintBAccounts]
    const entanglements = entanglementsAccounts.map(async (account) => await anchorProgram.account.entangledPair.fetch(account.pubkey).then((epObj) => epObj))
