...
export const getCandyMachine = async (
  ...
  const candyMachineCoder = new anchor.Coder(await anchor.Program.fetchIdl(
    CANDY_MACHINE_PROGRAM_ID, { connection: connection } as anchor.Provider));
...
export const validateEditionClaims = async (
  ...
  for (let idx = 0; idx < claimants.length; ++idx ) {
    ...
    const markerKey = await getEditionMarkPda(masterMint.key, edition.toNumber());
