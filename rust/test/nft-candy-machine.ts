...
describe("nft-candy-machine", function () {
  ...
  const createConfig = async function (
    ...
    that.vault = anchor.web3.Keypair.generate();
    that.authority = mintToVault ? that.vault : anchor.web3.Keypair.generate();
  ...
  describe("sol only", function () {
    beforeEach(async function () {
      ...
      this.config = config;
      const txInstr = await createConfig(this, false, false, 10);
  ...
  describe("sol only - pre-minting", function () {
    beforeEach(async function () {
      const config = await anchor.web3.Keypair.generate();
      this.config = config;
      const txInstr = await createConfig(this, false, true, 10);
      const linesInstr = await addConfigLines(this, 10);
      this.candyMachineUuid = anchor.web3.Keypair.generate()
        .publicKey.toBase58()
        .slice(0, 6);
      ...
      try {
        const tx = await program.rpc.initializeCandyMachine(
          ...
          },
          {
            ...
            signers: [myWallet, this.authority, this.config, this.vault],
            instructions: [
              anchor.web3.SystemProgram.createAccount({
                fromPubkey: myWallet.publicKey,
                newAccountPubkey: config.publicKey,
  ...
  describe("token - dynamic minting", function () {
    beforeEach(async function () {
      const config = await anchor.web3.Keypair.generate();
      this.config = config;
      const txInstr = await createConfig(this, true, false, 5);
      ...
      try {
        const tx = await program.rpc.initializeCandyMachine(
          ...
          },
          {
            ...
            signers: [myWallet, this.tokenMint, this.authority, this.config, this.vault],
            instructions: [
              anchor.web3.SystemProgram.createAccount({
                fromPubkey: myWallet.publicKey,
                newAccountPubkey: config.publicKey,
    ...
    it("mints without goLive date", async function () {
      ...
      try {
        const tx = await program.rpc.mintNft({
          ...
          signers: [mint, this.authority, myWallet, transferAuthority, this.vault],
          instructions: [
            // Give authority enough to pay off the cost of the nft!
            // it'll be funnneled right back
            anchor.web3.SystemProgram.transfer({
  ...
  describe("token - pre-minting", function () {
    beforeEach(async function () {
      const config = await anchor.web3.Keypair.generate();
      this.config = config;
      const txInstr = await createConfig(this, true, true, 5);
      const linesInstr = await addConfigLines(this, 5);
      this.tokenMint = anchor.web3.Keypair.generate();
      this.candyMachineUuid = anchor.web3.Keypair.generate()
        .publicKey.toBase58()
