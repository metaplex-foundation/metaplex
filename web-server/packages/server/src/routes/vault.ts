...
import { createMongoClient, METADATA_COLLECTION, DB, VAULTS_COLLECTION } from '../db/mongo-utils';
...
router.get('/:store/wallet/:wallet/vaultsToUnwind', async (req: Request, res : Response) => {
    const client = await createMongoClient();
    const coll = client.db(DB).collection(VAULTS_COLLECTION);
    const store = req.params.store;
